...
const JobContainer = ({ jobs, filterActive }) => {
    ...
    useEffect(() => setVisibleJobs(jobs.slice(9 * (activePage - 1), 9 * activePage)), [activePage, filterActive])
                    ...
                    visibleJobs.map((job, index) => {
                        return (
                            <div className="item" key={index}>
                                ...
                                {
                                    job.valid ? (
                                        <div className="apply my-4 d-flex justify-content-center">
                                            <Link href="/job/apply">Apply Now</Link>
                                        </div>
                                    ) : (
                                        <div className="apply my-4 d-flex justify-content-center unvaild">
                                            <span>Apply Now</span>
                                        </div>
                                    )
                                }
